...
With that we we have no our final features.
...
train <- splits[[1]]
test <- splits[[2]]
our_data <- read.csv("Our-data.csv")
```
...
```
```{r}
perf_best_RMSE <- h2o.performance(best_RMSE, test)
```
```{r}
...
exp_automl <- h2o.explain(automl, test) 
print(exp_automl) 
...
exp_GBM <- h2o.explain(GBM_auc, test)  # Explain the best performing GBM model 
print(exp_GBM)
...
# the validation set)
row_4_GBM_auc <- h2o.explain_row(exp_GBM, test, row_index = 4)
row_4_GBM_auc
...
Abi <- h2o.explain_row(GBM_auc, our_data, row_index = 4)
