...
# Check how many students have wage > 170'000
...
So we create now the standard function to replace the outliers with the 5th and 95th percentile value of that wage feature.
...
### Abishan
```{r}
Abishan <- h2o.explain_row(GBM_auc, test, row_index = 1)
Abishan
```
...
```{r}
Josua <- h2o.explain_row(GBM_auc, test, row_index = 2)
Josua
```
...
```{r}
Lars <- h2o.explain_row(GBM_auc, test, row_index = 3)
Lars
```
...
```{r}
Luca <- h2o.explain_row(GBM_auc, test, row_index = 4)
Luca
```
...
Across all 4 trials there are clear indications of what is relevant to wage. Clearly, country is a very strong factor. Also, a very important factor is whether you are a student or not. We also see the biggest difference in our statements, since Luca was the only one who did not state that he is in industry = I am a student. So he earns 3 times as much as the rest of us. Age is also a relevant factor. With our information (everyone under 30 years of age), this has a negative impact on wages. The not clearly assigned job role (Other) is the only strong factor that is not clearly comprehensible. We are therefore fundamentally very satisfied with our model, as the factors are very plausible and understandable. We are happy as soon as we can settle our student and immediately earn 3 times more than before ;)
